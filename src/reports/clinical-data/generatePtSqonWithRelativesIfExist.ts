...
type AggregationIdsRequest = {
    [index: string]: any;
    query: any;
    body: { aggregations: { ids: { buckets: Bucket[] } } };
};

export const extractFieldAggregationIds = async (
    query: any,
    field: string,
    searchExecutor: (q: any) => Promise<AggregationIdsRequest>,
): Promise<string[]> => {
    const r = await searchExecutor({
        query,
        aggs: { ids: { terms: { field: field, size: ES_QUERY_MAX_SIZE } } },
    });
...
const generatePtSqonWithRelativesIfExist = async (
    ...
    const searchExecutor = async (q: any) => await executeSearch(es, normalizedConfigs.alias, q);

    const allSelectedParticipantsIds: string[] = await extractFieldAggregationIds(
        query,
        configGlobal.participant_id,
        searchExecutor,
    );
    const allFamiliesIdsOfSelectedParticipants: string[] = await extractFieldAggregationIds(
        {
            bool: { must: [{ terms: { [configGlobal.participant_id]: allSelectedParticipantsIds } }] },
        },
        configGlobal.family_id,
        searchExecutor,
    );
    const allRelativesIds: string[] = await extractFieldAggregationIds(
        {
            bool: { must: [{ terms: { [configGlobal.family_id]: allFamiliesIdsOfSelectedParticipants } }] },
        },
        configGlobal.participant_id,
        searchExecutor,
    );
        ...
        content: [
            {
                op: 'in',
                content: {
                    field: configGlobal.participant_id,
